...
class UserService {
    ...
    async get(): Promise<QueryResult<any>> {
        ...
        try {
            const res = await client.query(`SELECT u.id, u.name, u.lastname, u.email, r.name AS roleName FROM   users AS u INNER JOIN roles AS r ON u.role = r.id;`);
            return res;
        } finally {
            client.release();
        }
    ...
    async find(id: Number): Promise<QueryResult<any>> {
        const client = await this.pool.connect();
        try {
            const res = await client.query("SELECT id,name,lastname,email,role FROM users WHERE id = $1", [id]);
            const user = res.rows[0];
            if (!user) {
